...
func (s *Server) activateLicense(w http.ResponseWriter, r *http.Request) {
	...
	if err := PersistLicense(s.dataDir, key); err != nil {
		log.Printf("fleet: license persist failed: %v", err)
	...
	s.limMu.Unlock()
	log.Printf("fleet: license activated via dashboard, persisted to %s/%s, tier=%s", s.dataDir, licenseFilename, newTier)
...
func (s *Server) listVehicles(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"vehicles": oe(s.db.SearchVehicles(q, filters))})
		return
	}
...
func (s *Server) updateVehicles(w http.ResponseWriter, r *http.Request) {
	...
	if patch.LicensePlate == "" {
		patch.LicensePlate = existing.LicensePlate
	}
	if patch.Status == "" {
...
func (s *Server) delVehicles(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	s.db.DeleteVehicles(id)
	s.db.DeleteExtras("vehicles", id)
...
func (s *Server) listMaintenance(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"maintenance": oe(s.db.SearchMaintenance(q, filters))})
		return
	}
...
func (s *Server) delMaintenance(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	s.db.DeleteMaintenance(id)
	s.db.DeleteExtras("maintenance", id)
	wj(w, 200, map[string]string{"deleted": "ok"})
}
